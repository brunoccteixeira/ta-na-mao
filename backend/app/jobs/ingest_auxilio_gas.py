"""Auxílio Gás data ingestion from Portal da Transparência.

Downloads official Auxílio Gás beneficiary data.
Source: https://portaldatransparencia.gov.br/download-de-dados/auxilio-gas

Auxílio Gás: Auxílio financeiro para compra de botijão de gás de cozinha
para famílias de baixa renda inscritas no CadÚnico. Pago bimestralmente.
"""
...
DIRECT_URL = "https://dadosabertos-download.cgu.gov.br/PortalDaTransparencia/saida/auxilio-gas"
...
async def download_auxilio_gas(year: int, month: int) -> Optional[bytes]:
    """Download Auxílio Gás ZIP file for a specific month."""
    period = f"{year}{month:02d}"
    url = f"{DIRECT_URL}/{period}_AuxilioGas.zip"

    logger.info(f"Downloading Auxílio Gás data for {period}...")
...
def parse_auxilio_gas_csv(zip_content: bytes, siafi_mapping: Dict[str, str]) -> Dict[str, Dict]:
    """Parse Auxílio Gás CSV from ZIP and aggregate by municipality."""
    results = {}
    ...
    with zipfile.ZipFile(zip_buffer) as zf:
        ...
        with zf.open(csv_name) as csv_file:
            ...
            for i, col in enumerate(header):
                col_upper = col.upper()
                if 'SIAFI' in col_upper or 'CÓDIGO MUNICÍPIO' in col_upper:
                    siafi_idx = i
                elif 'VALOR' in col_upper and 'PARCELA' in col_upper:
                    valor_idx = i
            ...
            if siafi_idx is None:
                logger.error(f"Municipality code column not found. Headers: {header[:15]}")
                return {}
...
def save_auxilio_gas_data(db: Session, data: Dict[str, Dict], reference_date: date):
    """Save Auxílio Gás data to database."""
    ...
    program = db.query(Program).filter(Program.code == ProgramCode.AUXILIO_GAS).first()
    if not program:
        logger.error("Auxílio Gás program not found in database. Run seed_programs first.")
        return 0, 0
    ...
    for ibge_code, mun_data in data.items():
        ...
        if existing:
            existing.total_beneficiaries = families
    ...
    return total_families, total_value


async def ingest_auxilio_gas(year: int, month: int):
    """Main function to ingest Auxílio Gás data."""
    logger.info("=" * 60)
    logger.info(f"INGESTING AUXÍLIO GÁS DATA - {year}/{month:02d}")
    logger.info("=" * 60)

    siafi_mapping = load_siafi_mapping()

    zip_content = await download_auxilio_gas(year, month)
    ...
    data = parse_auxilio_gas_csv(zip_content, siafi_mapping)
    ...
    db = SessionLocal()
    try:
        total_families, total_value = save_auxilio_gas_data(db, data, reference_date)

        logger.info("=" * 60)
        logger.info("INGESTION COMPLETE!")
        logger.info(f"Auxílio Gás families: {total_families:,}")
        logger.info(f"Total value: R$ {total_value:,.2f}")
        logger.info("=" * 60)
...
def run_ingestion():
    ...
    asyncio.run(ingest_auxilio_gas(year, month))
...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 3:
        ...
        print("Usage: python -m app.jobs.ingest_auxilio_gas [YEAR] [MONTH]")
        print("")
        print("Examples:")
        print("  python -m app.jobs.ingest_auxilio_gas 2024 10")
        print("")
        print("Data source: Portal da Transparência - Auxílio Gás")
        print("URL: https://portaldatransparencia.gov.br/download-de-dados/auxilio-gas")
    else:
        run_ingestion()
