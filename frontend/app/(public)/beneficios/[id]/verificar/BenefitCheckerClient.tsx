'use client';

import { useState, useCallback, useEffect } from 'react';
import Link from 'next/link';
import { useRouter } from 'next/navigation';
import { ArrowLeft } from 'lucide-react';
import { useBenefitDetail } from '../../../../../src/hooks/useBenefitsAPI';
import { BenefitEligibilityChecker } from '../../../../../src/components/BenefitChecker';
import type { CitizenProfile } from '../../../../../src/engine/types';

const STORAGE_KEY = 'tnm_profile';

export default function BenefitCheckerClient({ id }: { id: string }) {
  const router = useRouter();
  const { data: benefit, isLoading, isError, error } = useBenefitDetail(id);

  // Lazy-load profile from sessionStorage to avoid SSR mismatch
  const [profile, setProfile] = useState<CitizenProfile | null>(null);

  useEffect(() => {
    try {
      const raw = sessionStorage.getItem(STORAGE_KEY);
      if (raw) setProfile(JSON.parse(raw) as CitizenProfile);
    } catch {
      // sessionStorage may be disabled
    }
  }, []);

  const handleProfileSubmit = useCallback((newProfile: CitizenProfile) => {
    try {
      sessionStorage.setItem(STORAGE_KEY, JSON.stringify(newProfile));
    } catch {
      // sessionStorage may be disabled
    }
    setProfile(newProfile);
  }, []);

  if (isLoading) {
    return (
      <div className="min-h-screen bg-[var(--bg-primary)] flex items-center justify-center">
        <div className="flex items-center gap-3 text-[var(--text-secondary)]">
          <div className="w-5 h-5 border-2 border-emerald-500 border-t-transparent rounded-full animate-spin" />
          <span>Carregando...</span>
        </div>
      </div>
    );
  }

  if (isError || !benefit) {
    return (
      <div className="min-h-screen bg-[var(--bg-primary)] flex items-center justify-center px-4">
        <div className="text-center">
          <p className="text-[var(--text-secondary)] mb-4">
            {error instanceof Error ? error.message : 'Beneficio nao encontrado'}
          </p>
          <Link
            href="/beneficios"
            className="text-emerald-600 hover:text-emerald-700 font-medium"
          >
            Voltar ao catalogo
          </Link>
        </div>
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-[var(--bg-primary)]">
      {/* Header */}
      <header className="sticky top-0 z-30 bg-white/95 border-b border-[var(--border-color)]">
        <div className="max-w-lg mx-auto px-4 py-3 flex items-center gap-3">
          <button
            onClick={() => router.back()}
            className="w-12 h-12 min-w-[48px] rounded-full bg-[var(--bg-primary)] flex items-center justify-center transition-colors hover:bg-gray-200"
            aria-label="Voltar"
          >
            <ArrowLeft className="w-5 h-5 text-[var(--text-primary)]" />
          </button>
          <div className="flex-1 min-w-0">
            <h1 className="text-base font-bold text-[var(--text-primary)] truncate">
              Verificar elegibilidade
            </h1>
            <p className="text-xs text-[var(--text-tertiary)] truncate">
              {benefit.name}
            </p>
          </div>
          <Link
            href="/"
            className="text-sm font-semibold text-emerald-600"
          >
            Ta na Mao
          </Link>
        </div>
      </header>

      {/* Content */}
      <main className="max-w-lg mx-auto px-4 py-6 pb-12">
        <BenefitEligibilityChecker
          benefit={benefit}
          profile={profile}
          onProfileSubmit={handleProfileSubmit}
        />
      </main>
    </div>
  );
}
